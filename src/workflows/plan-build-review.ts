    ...
    },
    {
      ...
      request: (ctx) => {
        ...
        return {
          ...
          instructions: `Read the inputs, fix every blocking finding in the repository, and keep the acceptance criteria satisfied.${enteredByReview ? ` ${REVIEW_IS_CANONICAL}` : ""} ${FOLLOW_THE_PLAN} ${COMPLETION_REPORT}`,
          inputs,
          task: ctx.input.task,
          ...(ctx.input.instructions?.builder !== undefined
            ? { roleInstructions: ctx.input.instructions.builder }
