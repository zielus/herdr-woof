...
Revision: `5e0236e6d06152222e6ff35f80dc05889841d4c2` · Date: 2026-09-16
`bun run verify`: green (724 tests, run twice at this head)
...
| Cancellation       | `process` + `live`      | `test/scheduler.process.test.ts` "…9. cancellation stops owned panes and a late result cannot resurrect the run"; `test/run-build-review.cli.test.ts` "runs exits 6 on SIGTERM, records the cancellation and refuses a late submission"; `runtime-loss-live.log:L7`                                                                                                                                                                                                                                                                                          | `bun x vitest run test/scheduler.process.test.ts test/run-build-review.cli.test.ts`                                          | PASS. `woof run cancel` recorded the L7 kill cleanly (`{"outcome":"recorded", ...}`), no new dispatch, no resurrection by a late result.                                                                                                                                                                                                                                 |
