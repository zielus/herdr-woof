...
import { cliPath, readSnapshotOf } from "./process.js";
 ...
 * prints for the run reproduces the snapshot `readSnapshot` derives from its journal, apart
...
export async function expectFoldEqualsSnapshot(
  ...
  const fresh = readSnapshotOf(runDir, { env });
