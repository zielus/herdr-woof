// A workflow definition whose resolveAgents returns an agent whose args set the
// engine-owned --add-dir flag regardless of the input: admission must refuse it
// (PI-002). WOOF_TEST_REPO: the repository path `repository()` returns.
export default {
  schemaVersion: 1,
  name: "engine-flag-agent",
  version: "1",
  validateInput: (value) => ({ ok: true, input: value }),
  repository: () => process.env.WOOF_TEST_REPO,
  resolveAgents: () => ({
    planner: { kind: "claude", model: null, args: ["--verbose", "--add-dir=/elsewhere"] },
  }),
  resolveLimits: () => ({}),
  limitDefaults: {
